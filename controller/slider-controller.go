...
type SliderController interface {
	AddSlider(ctx *gin.Context) error
	...
	DeleteSlider(ctx *gin.Context) error
...
func (c *sliderController) AddSlider(ctx *gin.Context) error /*entity.Video*/ {
	var slider entity.Slider
	err := ctx.ShouldBindJSON(&slider)
	if err != nil {
		return err
	}
...
func (c *sliderController) DeleteSlider(ctx *gin.Context) error {
	var sliderId entity.SliderEditRequest
	err := ctx.ShouldBindJSON(&sliderId)
	if err != nil {
		return err
	...
	for i := 0; i < len(stores); i++ {
		if stores[i].ID == sliderStoreId.StoreId {
			...
			if len(storeSliders) > 0 {
				...
				return storeSliders, errors.New("store does not have any product categories")
			}
		}
	}
